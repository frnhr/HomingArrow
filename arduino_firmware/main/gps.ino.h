...
void gps_loop()
{
    ...
    if (_gps.override) {
        ...
        if (!_gps.sensor.newNMEAreceived())
          got_new_data = false;

        // succesfully parsed the data:
        if (!_gps.sensor.parse(_gps.sensor.lastNMEA()))
          got_new_data = false;
        ...
        // use hardcoded home coords until first GPS fix:
        if (!got_new_data && !gps.inited) {
          gps.current_lat = deg2rad(GPS_HOME_LAT);
          gps.current_lon = deg2rad(GPS_HOME_LON);
        }
